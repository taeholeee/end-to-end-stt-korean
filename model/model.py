...
class LASModel(BaseModel):
    def __init__(self, max_label_len, input_feature_dim, listener_hidden_dim, listener_layer, multi_head, decode_mode, use_mlp_in_attention, 
                 mlp_dim_in_attention, mlp_activate_in_attention, speller_rnn_layer, speller_hidden_dim, output_class_dim, rnn_unit, use_gpu):
        super().__init__()
        self.max_label_len = max_label_len
        self.listener = Listener(input_feature_dim, listener_hidden_dim, listener_layer, rnn_unit, use_gpu)
        self.speller = Speller(output_class_dim,  speller_hidden_dim, rnn_unit, speller_rnn_layer, use_gpu, self.max_label_len,
                 use_mlp_in_attention, mlp_dim_in_attention, mlp_activate_in_attention, listener_hidden_dim, multi_head, decode_mode)

    def forward(self, batch_data, batch_label=None, tf_rate=0.9):
        ...
        pred_y = (torch.cat([torch.unsqueeze(each_y,1) for each_y in raw_pred_seq],1)[:,:self.max_label_len,:]).contiguous()
        return pred_y
